...
DisjSetADT NewDisjointSets() {
    DisjSetADT ds = malloc(sizeof(*ds));
    for (int i=100; i>0; i--) ds->class[i] = 0;
    return ds;
}
...
void SetUnion(DisjSetADT S, int X1, int X2) {
    ...
    if (R1 != R2) {
        if (-(S->class[R1]) < -(S->class[R2]))S->class[R1] = R2; 
        else { 
            if (S->class[R1] == S->class[R2])(S->class[R1])--; 
            S->class[R2] = R1;
        }
    }
}
